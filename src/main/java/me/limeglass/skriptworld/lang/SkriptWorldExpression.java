...
import me.limeglass.skriptworld.utils.annotations.DetermineSingle;
import me.limeglass.skriptworld.utils.annotations.Events;
import me.limeglass.skriptworld.utils.annotations.Multiple;
import me.limeglass.skriptworld.utils.annotations.Settable;
import me.limeglass.skriptworld.utils.annotations.Single;

public abstract class SkriptWorldExpression<T> extends SimpleExpression<T> implements DataChecker {
	...
	public boolean init(Expression<?>[] expressions, int matchedPattern, Kleenean isDelayed, ParseResult parser) {
		if (getClass().isAnnotationPresent(Events.class)) {
			if (!ScriptLoader.isCurrentEvent(getClass().getAnnotation(Events.class).value())) {
				SkriptWorld.debugMessage("The expression `" + getClass().getSimpleName() + "` can't be used in the event: " + ScriptLoader.getCurrentEventName());
				return false;
			}
		}
	...
	public String toString(Event event, boolean debug) {
		...
		if (event != null) SkriptWorld.debugMessage(getClass().getSimpleName() + " - " + modSyntax + " (" + event.getEventName() + ")" + " Data: " + Arrays.toString(values.toArray()));
