...
import me.limeglass.skriptworld.utils.annotations.Properties;
import me.limeglass.skriptworld.utils.annotations.Settable;

public abstract class SkriptWorldPropertyExpression<F, T> extends PropertyExpression<F, T> {
	...
	public String toString(Event event, boolean debug) {
		...
		return SkriptWorld.getNameplate() + getClass().getSimpleName() + ": the " + getPropertyName() + " (of|from) " + getExpr().toString(event, debug);
	}
	...
	protected Boolean isNull(Event event) {
		...
		if (getExpr().isSingle() && getExpr().getSingle(event) == null) {
			SkriptWorld.debugMessage("The property expression was null: " + getExpr().toString(event, true));
			return true;
