...
public abstract class SkriptWorldCondition extends Condition implements DataChecker {
	...
	public String toString(Event event, boolean debug) {
		...
		if (event == null) {
			SkriptWorld.debugMessage(getClass().getSimpleName() + " - " + modSyntax);
		} else {
			Arrays.asList(expressions.getExpressions()).stream().forEach(expression->values.add(expression.toString(event, debug)));
			SkriptWorld.debugMessage(getClass().getSimpleName() + " - " + modSyntax + " (" + event.getEventName() + ")" + " Data: " + Arrays.toString(values.toArray()));
		}
		return SkriptWorld.getNameplate() + getClass().getSimpleName() + "- Syntax: " + getSyntax();
	}
