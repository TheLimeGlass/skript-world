...
public class EnumClassInfo<E extends Enum<E>> {
	...
	public void register() {
		if (Classes.getExactClassInfo(enumType) == null) {
			if (SkriptWorld.getInstance().getConfig().getBoolean("UseEnglishEnums")) {
				...
				SkriptWorld.getSyntaxData().set("Syntax.Enums." + enumType.getSimpleName() + ".enums", enums);
			}
	...
	private void enumDebug(String message, String codeName, Class<?> clazz) {
		if (codeName.equalsIgnoreCase(SkriptWorld.getInstance().getConfig().getString("syntaxDebug"))) SkriptWorld.debugMessage("&eRegistered syntaxDebug found enum classinfo: " + codeName + " (" + clazz.getCanonicalName() + ")");
		else SkriptWorld.debugMessage("&5Registered Enum '" + codeName + "' with return class " + enumType.getName());
		SkriptWorld.getSyntaxData().set("Syntax.Enums." + clazz.getSimpleName() + ".user", user);
		Syntax.save();
	}
}