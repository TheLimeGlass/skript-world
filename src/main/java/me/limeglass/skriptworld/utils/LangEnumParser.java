...
import me.limeglass.skriptworld.lang.SkriptWorldParser;

public class LangEnumParser<T extends Enum<T>> extends SkriptWorldParser<T> {
	...
	public LangEnumParser(String variableNamePattern, Class<T> clazz) {
		...
		try {
			...
			SkriptWorld.getSyntaxData().set("Syntax.Enums." + clazz.getSimpleName() + ".names", enumNames);
		} catch (NullPointerException error) {
			SkriptWorld.consoleMessage("&cThe class: " + clazz.getName() +  " for classinfo name: " + variableNamePattern + " isn't an Enum!");
		}
		Syntax.save();
	}
